...
from modified_ai_economist_wt.foundation.base.base_env import BaseEnvironment, scenario_registry
from modified_ai_economist_wt.foundation.scenarios.utils import rewards, social_metrics
...
class UniformVoteAndInvest(BaseEnvironment):
    ...
    def reset_agent_states(self):
        ...
        count = int(0.5 * self.n_agents)
        index_teacher = np.random.permutation(self.n_agents)[:count]

        if self.n_agents == 6:
            self.world.agents[0].state["endogenous"]["Teaching"] = ['teacher', communication_resource_words]
            self.world.agents[1].state["endogenous"]["Teaching"] = ['teacher', communication_resource_words]
            self.world.agents[2].state["endogenous"]["Teaching"] = ['teacher', communication_resource_words]
            self.world.agents[3].state["endogenous"]["Teaching"] = ['student', ['d', 'a', 'b', 'c']]
            self.world.agents[4].state["endogenous"]["Teaching"] = ['student', ['c', 'd', 'a', 'b']]
            self.world.agents[5].state["endogenous"]["Teaching"] = ['student', ['b', 'c', 'd', 'a']]
        
        else:
            for agent in self.world.agents:
                if int(agent.idx) in index_teacher:
                    agent.state["endogenous"]["Teaching"] = ['teacher', communication_resource_words]
                else:
                    index = np.random.permutation(4)
                    agent.state["endogenous"]["Teaching"] = ['student', [communication_resource_words[index[0]], communication_resource_words[index[1]], communication_resource_words[index[2]], communication_resource_words[index[3]]]] 
        ...
        self.world.planner.state["endogenous"]["Teaching"] = []
